...
# --- 2. 화면 구성 (UI) ---
st.set_page_config(page_title="자녀 양육비 계산기", page_icon="⚖️")

# [수정된 타이틀 부분]
st.title("🧮 자녀 양육비 계산기")
st.markdown("##### 창원·경남 **나자현 변호사**가 **이혼시 자녀 양육비가 궁금한 분들**을 위해 직접 만들었습니다.")
...
with st.container():
    ...
    with col1:
        my_income = st.number_input("양육자 월 소득 (단위: 만원)", min_value=0, step=10, value=200, help="세전 소득 기준")
        st.caption(f"👉 입력 확인: **{my_income * 10000:,}원**")
        
    with col2:
        ex_income = st.number_input("비양육자 월 소득 (단위: 만원)", min_value=0, step=10, value=300, help="세전 소득 기준")
        st.caption(f"👉 입력 확인: **{ex_income * 10000:,}원**")
...
st.markdown("---")
st.markdown("**Created by Lawyer NajaHyun (Changwon/Gyeongnam)**")
